...
import { AwesomeHTTPSensorAccessory } from "./aweSomeSensorAccessory";
...
class AwesomePlatform implements StaticPlatformPlugin {
  ...
  constructor(log: Logging, config: PlatformConfig, api: API) {
   ...
   log.debug("API " + api.serverVersion);
    ...
    if (!this.aweSomeConfig.sensors) {
      log.info("No config for sensors yet. Go to settings and start configuring");
    }
  ...
  accessories(callback: (foundAccessories: AccessoryPlugin[]) => void): void {
    ...
    if (!this.aweSomeConfig.sensors) {
      this.log.info("No config for sensors yet. Return no accessory.");
      callback([]);
    } else {
      callback([
        new AwesomeHTTPSensorAccessory(hap, this.log, this.config),
      ]);
    }
  }
